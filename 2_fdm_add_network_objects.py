...
def get_existing_networks():
    ...
    while go:    
        ...
        objects = fdm_get(FDM_HOST,token,api_path,FDM_VERSION,FDM_USER,FDM_PASSWORD,offset,limit_global)        
        #print(output)
        index=0
...
if __name__ == "__main__":
    ...
    list=[]
    list=read_csv("./objects_csv_files/network_objects.csv")
    print (yellow("OK DONE ( Step 2 )",bold=True))    
    print('====================================================================================================')    
    print (yellow("Finally let's POST every new objects into FTD Device"))    
    for object in list:
        print(object)    
        token=new_auth_token[0]
        post_response = fdm_create_network(FDM_HOST,token,object,FDM_VERSION,FDM_USER,FDM_PASSWORD)
