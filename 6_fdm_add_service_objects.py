...
def fdm_create_service(host,token,payload,version,username,password):
    ...
    try:
        if payload['protocol']=='tcp':
            ...
            # print(payload)
            api_url="https://{}:{}/api/fdm/v{}/object/tcpports".format(host, FDM_PORT,version)
            ...
            del payload['protocol']
            print(payload) 
            api_url="https://{}:{}/api/fdm/v{}/object/udpports".format(host, FDM_PORT,version)
            request = requests.post(api_url,json=payload, headers=headers, verify=False)
            status_code = request.status_code
            if status_code == 401: # Token is invalid !
                print(red("Auth Token invalid, Let\'s ask for a new one",bold=True))
...
if __name__ == "__main__":
    ...
    print('======================================================================================================================================')    
    print (yellow("Second let's read the csv file and add new objects into a temporay list"))
    list=[]
    list=read_csv("./objects_csv_files/service_objects.csv")
