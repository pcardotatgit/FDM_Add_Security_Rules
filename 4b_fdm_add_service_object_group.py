...
def get_port_types(token,version,username,password):
	...
	try:				
		...
		ports = fdm_get(FDM_HOST,token,api_url,version,username,password,offset)
...
def get_port_types_to_debug(token,version,username,password):
	'''
		retrieve all port types
	'''
...
if __name__ == "__main__":
	...
	ii=0
	while go==1:	
		servces = fdm_get(FDM_HOST,token,api_url,FDM_VERSION,FDM_USER,FDM_PASSWORD,offset)
		#print(servces)
		ii=0
		if servces.get('items'):
			for line in servces['items']:
	...
	while go==1:		
		...
		if ii>=999:
			...
			go=0	
	fi = open("existing_servce_objects.txt", "w")	
	...
	list=[]
	list=read_csv("port_object_groups.csv",token,FDM_VERSION,FDM_USER,FDM_PASSWORD)
